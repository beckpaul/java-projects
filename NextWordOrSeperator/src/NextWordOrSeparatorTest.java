...
public final class NextWordOrSeparatorTest {
    ...
    private static String nextWordOrSeparator(String text, int position,
        ...
        if (separators.contains(text.charAt(position))) {
            return returnString += text.charAt(position);
        } else {

            for (int i = 0; i < text.substring(position, text.length())
                    .length(); ++i) {
                test = text.charAt(position + i);

                if (!separators.contains(test)) {
                    returnString += test;
                } else {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(separatorSet);
        /*
