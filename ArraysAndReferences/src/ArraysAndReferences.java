...
public final class ArraysAndReferences {
    ...
    private static NaturalNumber productOfArrayElements(
        ...
        NaturalNumber sum = new NaturalNumber2(nnArray[0]);
        for (int i = 1; i < nnArray.length; i++) {
            NaturalNumber tmp = new NaturalNumber2(nnArray[i]);
            sum.multiply(tmp);
        }
    ...
    private static void computePartialProducts(NaturalNumber[] nnArray) {
        ...
        for (int i = 0; i < nnArray.length; i++) {
            ...
            for (int y = i; y >= 0; y--) {
                NaturalNumber sum = new NaturalNumber2(y);
                sum = nnArray[
            }
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < array.length; i++) {
            NaturalNumber tmp = new NaturalNumber2(count);
            array[i] = tmp;
            count.increment();
        }
