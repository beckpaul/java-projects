...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!in.atEOS()) {
            ...
            String price = tmp.substring(commaIndex + 1);
            String item = tmp.substring(0, commaIndex);
            Integer priceValue = Integer.parseInt(price);

            priceMap.add(item, priceValue);
        }
        in.close();
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        while (tmp != "") {
            ...
            if (sizePriceMap.hasKey(tmp)) {
                val += sizePriceMap.value(tmp);
            }
            if (toppingPriceMap.hasKey(tmp)) {
                val += toppingPriceMap.value(tmp);
            }
            tmp = input.nextLine();
        }
     ...
     * </pre>
     */
    private static void putPrice(SimpleWriter out, int price) {
